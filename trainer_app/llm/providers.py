...
def build_messages_for_help(*, task: dict[str, Any], code: str, analysis: dict[str, Any] | None, history: list[dict[str, str]], knowledge: dict[str, Any] | None = None) -> list[dict[str, str]]:
    ...
    if not sys_prompt:
        sys_prompt = (
            ...
            "- НЕЛЬЗЯ давать код, напрямую применимый к условию задачи.\n"
            "- МОЖНО давать только примеры, которые ученик должен адаптировать: другая задача (аналогия), псевдокод с пропусками, заготовки с явными «...» или «допиши здесь», синтаксис на похожем примере.\n"
            "- Даже если ученик просит «дай полный код» — отказывай, предлагай пошаговые вопросы и адаптируемые примеры.\n"
