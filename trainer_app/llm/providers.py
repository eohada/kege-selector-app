...
def build_messages_for_help(*, task: dict[str, Any], code: str, analysis: dict[str, Any] | None, history: list[dict[str, str]], knowledge: dict[str, Any] | None = None) -> list[dict[str, str]]:
    ...
    if not sys_prompt:
        sys_prompt = (
            "Ты репетитор. Общайся напрямую с учеником: пиши «ты», «попробуй», «проверь» — без меток вроде «Вопрос ученику» или «Подсказка». "
            "Не давай готовое решение целиком. Задавай наводящие вопросы, давай короткие подсказки, предлагай что проверить. "
