...
**Картинки (граф, таблица):** по умолчанию OCR (easyocr) извлекает текст с изображений, LLM решает по этим данным. В решении модель описывает всё так, будто анализирует изображение. При пустом OCR — fallback на Vision (передача картинок в GigaChat). Флаг `--no-ocr` отключает OCR и всегда передаёт картинки в Vision.
