...
### Сохранение аватарок после деплоя (опционально):
Файловая система контейнера эфемерная — загруженные аватарки пропадают после обновления. Чтобы они сохранялись:
- Создайте **Volume** в Railway для сервиса приложения и примонтируйте его, например, в `/data`.
- Задайте переменную: `AVATAR_UPLOAD_ROOT=/data/uploads/avatars`.
- При первой загрузке аватарки папка создастся автоматически; файлы будут раздаваться по URL `/avatars/...`.
