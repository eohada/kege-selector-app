...
### Сохранение аватарок после деплоя (обязательно, иначе аватарки сбрасываются при каждом редеплое):
**Без этой настройки загруженные аватарки пропадают после каждого деплоя**, т.к. файловая система контейнера эфемерная.
1. В Railway: сервис приложения → **Volumes** → **Add Volume**. Путь монтирования, например: `/data`.
2. В **Variables** добавьте: `AVATAR_UPLOAD_ROOT=/data/uploads/avatars` (если volume смонтирован в `/data`).
3. Сохраните и задеплойте заново. Папка создастся при первой загрузке аватарки; файлы будут отдаваться по URL `/avatars/...`.
