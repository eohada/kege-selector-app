...
- `TRAINER_URL` - для тренажёра: полный URL приложения тренажёра (Streamlit), например `https://trainer.boostudy.ru` (без слэша в конце)

### Тренажёр: 404 на /trainer или /trainer/

**Симптом:** страница «ТУТ НИЧЕГО НЕТ» при открытии тренажёра по ссылке `/trainer` или `/trainer/`.

**Что проверить:**
1. В коде маршрут принимает оба варианта (`/trainer` и `/trainer/`). Убедитесь, что задеплоена последняя версия.
2. На новом хостинге задайте переменную **`TRAINER_URL`** — полный URL сервиса тренажёра (Streamlit), например `https://trainer.boostudy.ru`. Без неё страница тренажёра откроется, но iframe будет пустой с сообщением «TRAINER_URL не задан».
3. Если перед приложением стоит nginx/прокси — убедитесь, что запросы к `/trainer` и `/trainer/` проксируются на Flask-приложение, а не отдают 404 на уровне прокси.
