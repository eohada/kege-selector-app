...
from wtforms import StringField, TextAreaField, SelectField, IntegerField, SubmitField, DateTimeLocalField, DateField, BooleanField, SelectMultipleField
...
class CourseModuleForm(FlaskForm):
    ...
    is_control_exam = BooleanField('Контрольный модуль / Пробный экзамен', default=False)
    submit = SubmitField('Сохранить')
...
class CourseLessonForm(FlaskForm):
    ...
    lesson_format = SelectField('Формат урока', choices=[
        ('Теория + Практика', 'Теория + Практика'),
        ('Практикум', 'Практикум'),
        ('Разбор ДЗ / Пробника', 'Разбор ДЗ / Пробника'),
        ('Контрольный урок / Пробник', 'Контрольный урок / Пробник'),
    ], default='Теория + Практика', validators=[DataRequired()])
    lesson_type = SelectField('Тип урока', choices=[
        ('regular', 'Обычный урок'),
        ('exam', 'Проверочный / Пробник'),
        ('introductory', 'Вводный урок'),
    ], default='regular', validators=[Optional()])
    status = SelectField('Статус', choices=[
        ('draft', 'Черновик'),
        ('planned', 'В плане'),
    ...
    studio_scenario = TextAreaField('Сценарий Studio', validators=[Optional(), Length(max=12000)])
    content = TextAreaField('Теория и материалы к уроку', validators=[Optional(), Length(max=16000)])
    homework = TextAreaField('Домашнее задание', validators=[Optional(), Length(max=12000)])
