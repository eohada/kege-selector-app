...
from app.models import UserProfile, FamilyTie, Enrollment, Student, BotAdmin, BotErrorReport, UserNotification
...
@admin_bp.route('/admin/bot')
@login_required
def admin_bot_panel():
    """Админка Telegram-бота: админы, рассылка, ошибки."""
    if not (current_user.is_admin() or current_user.is_creator()):
        flash('Доступ запрещен. Требуется роль "Администратор" или "Создатель".', 'danger')
        return redirect(url_for('main.dashboard'))
...
@admin_bp.route('/admin/bot/admins/add', methods=['POST'])
@login_required
def admin_bot_admins_add():
    """Добавить администратора бота."""
    if not (current_user.is_admin() or current_user.is_creator()):
        flash('Доступ запрещен.', 'danger')
        return redirect(url_for('admin.admin_bot_panel'))
    ...
    if admin:
        ...
        admin = BotAdmin(user_id=user.id, created_by_user_id=current_user.id, is_active=True)
        db.session.add(admin)
    
    try:
        db.session.commit()
        flash(f'Админ бота добавлен: {user.username}', 'success')
    except Exception as e:
        db.session.rollback()
...
@admin_bp.route('/admin/bot/admins/<int:admin_id>/toggle', methods=['POST'])
@login_required
def admin_bot_admins_toggle(admin_id: int):
    """Включить/выключить администратора бота."""
    if not (current_user.is_admin() or current_user.is_creator()):
        flash('Доступ запрещен.', 'danger')
        return redirect(url_for('admin.admin_bot_panel'))
    ...
    try:
        db.session.commit()
        flash('Статус администратора обновлен.', 'success')
    except Exception as e:
        db.session.rollback()
...
@admin_bp.route('/admin/bot/admins/<int:admin_id>/delete', methods=['POST'])
@login_required
def admin_bot_admins_delete(admin_id: int):
...
@admin_bp.route('/admin/bot/broadcast', methods=['POST'])
@login_required
def admin_bot_broadcast():
    """Рассылка новостей платформы в Telegram."""
    if not (current_user.is_admin() or current_user.is_creator()):
        flash('Доступ запрещен.', 'danger')
        return redirect(url_for('admin.admin_bot_panel'))
    ...
    try:
        ...
        audit_logger.log(
            ...
            entity_id=None,
            status='success',
            metadata={'count': count, 'title': title},
        )
        flash(f'Рассылка создана: {count} получателей.', 'success')
    except Exception as e:
        db.session.rollback()
        flash(f'Ошибка при создании рассылки: {e}', 'danger')
    
    return redirect(url_for('admin.admin_bot_panel'))


@admin_bp.route('/admin/bot/errors/<int:report_id>/reply', methods=['POST'])
@login_required
def admin_bot_error_reply(report_id: int):
    ...
    report.admin_user_id = current_user.id
    ...
    try:
        db.session.commit()
        flash('Ответ сохранен и будет отправлен пользователю.', 'success')
    except Exception as e:
        db.session.rollback()
        flash(f'Ошибка при сохранении ответа: {e}', 'danger')
    
    return redirect(url_for('admin.admin_bot_panel'))


@admin_bp.route('/admin/bot/errors/<int:report_id>/status', methods=['POST'])
@login_required
def admin_bot_error_status(report_id: int):
    ...
    try:
        ...
        if all_enrollments:
            ...
            for e in enrollments:
                if e.tutor_id not in user_ids or e.student_id not in user_ids:
                    continue
...
@admin_bp.route('/admin/api/family-ties', methods=['POST'])
@login_required
def admin_api_family_tie_create():
    """AJAX: создать FamilyTie (для графа)."""
    if not (current_user.is_admin() or current_user.is_creator()):
        return jsonify({'success': False, 'error': 'Access denied'}), 403
    ...
    try:
        ...
        db.session.rollback()
        logger.error(f"Error in admin_api_family_tie_create: {e}", exc_info=True)
        return jsonify({'success': False, 'error': str(e)}), 500


@admin_bp.route('/admin/api/enrollments', methods=['POST'])
@login_required
def admin_api_enrollment_create():
    """AJAX: создать Enrollment (для графа)."""
    if not (current_user.is_admin() or current_user.is_creator()):
        return jsonify({'success': False, 'error': 'Access denied'}), 403
    ...
    try:
        ...
        db.session.rollback()
        logger.error(f"Error in admin_api_enrollment_create: {e}", exc_info=True)
        return jsonify({'success': False, 'error': str(e)}), 500
