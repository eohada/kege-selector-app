...
def student_dashboard():
    ...
    from app.models import StudentLearningPlanItem, Submission, UserNotification, GradebookEntry, Assignment, Lesson, StudentCourseEnrollment, LearningTrajectory, LearningItem
    ...
    if student and selected_course_id:
        try:
            selected_course = LearningTrajectory.query.filter_by(course_id=selected_course_id, student_id=student.student_id).first()
            if selected_course:
                course_weekly_items = LearningItem.query.filter_by(course_id=selected_course.course_id).filter(
                    LearningItem.status.in_(['planned', 'in_progress', 'overdue'])
                ).order_by(LearningItem.due_at.asc().nullslast(), LearningItem.order_index.asc()).limit(5).all()
        except Exception:
            course_weekly_items = []
    ...
    return render_template(
        ...
        course_weekly_items=course_weekly_items,
    )
...
def _guidebook_sections(audience):
    common = [
        {'slug': 'start', 'icon': 'ph-rocket-launch', 'title': 'Быстрый старт', 'summary': 'Как войти, выбрать рабочую роль и не потеряться в навигации.', 'steps': [('Откройте нужный раздел', 'Используйте боковую навигацию: она меняется под роль и ведёт только на актуальные V2-экраны.'), ('Проверьте контекст', 'В шапке страницы смотрите имя ученика, курс или урок — это помогает не изменить данные не того человека.'), ('Сохраняйте результат', 'После любого действия дождитесь уведомления об успешном сохранении и только потом переходите дальше.')], 'links': [('Открыть дашборд', '/dashboard')]},
        {'slug': 'telegram', 'icon': 'ph-telegram-logo', 'title': 'Telegram и уведомления', 'summary': 'Привязка аккаунта, уведомления о занятиях и безопасное открытие Mini App.', 'steps': [('Создайте код привязки', 'В профиле откройте блок Telegram и сгенерируйте одноразовый код.'), ('Откройте бота', 'Перейдите по ссылке из профиля, нажмите Start и отправьте код без лишних пробелов.'), ('Проверьте статус', 'Вернитесь в профиль: состояние должно измениться на «Подключён».'), ('Настройте расписание', 'В Telegram Mini App откройте расписание и проверьте часовой пояс.'), ('Управляйте уведомлениями', 'Напоминания о занятиях и дедлайнах приходят в Telegram; отключить связь можно из профиля.')], 'links': [('Профиль', '/workspace/profile'), ('Расписание', '/schedule')]},
    ]
    teacher = [
        {'slug': 'students', 'icon': 'ph-users-three', 'title': 'Ученики и отношения', 'summary': 'Добавление ученика, приглашение, наставник и переход к его рабочему контексту.', 'steps': [('Откройте список учеников', 'В карточке ученика доступны дашборд, профиль и связь с наставником.'), ('Создайте приглашение', 'Нажмите «Пригласить ученика», сформируйте ссылку и отправьте её ученику.'), ('Проверьте связь', 'После регистрации убедитесь, что ученик отображается в вашем списке и имеет назначенного преподавателя.')], 'links': [('Список учеников', '/students')]},
        {'slug': 'courses', 'icon': 'ph-map-trifold', 'title': 'Курсы и маршрут', 'summary': 'Как собрать индивидуальную программу от навыков до запуска урока.', 'steps': [('Создайте курс', 'Укажите ученика, предмет, цель и ожидаемый результат.'), ('Настройте навыки', 'Добавьте проверяемые умения: именно они становятся основой диагностики и прогресса.'), ('Соберите маршрут', 'В мастере задайте цель, режим обучения и стартовый уровень, затем запустите генерацию.'), ('Запустите урок', 'В карточке запланированного занятия нажмите «Начать занятие» — откроется V2-комната урока.')], 'links': [('Курсы ученика', '/students'), ('Управление навыками', '/students')]},
        {'slug': 'lesson-room', 'icon': 'ph-video-camera', 'title': 'Комната индивидуального урока', 'summary': 'Единый процесс урока: видео, практика, теория, доска, материалы и статусы.', 'steps': [('Подключите видео', 'Откройте видео-Daily в верхней панели и проверьте камеру и микрофон.'), ('Ведите этап', 'Переключайте подготовку, практику и итог в маршруте урока; состояние сохраняется в комнате.'), ('Работайте вместе', 'Используйте доску для объяснения, практику для кода, материалы для файлов и задания для фокуса.'), ('Завершите урок', 'Зафиксируйте итог, комментарии и следующий шаг ученика.')], 'links': [('Расписание', '/schedule')]},
        {'slug': 'assignments', 'icon': 'ph-clipboard-text', 'title': 'Работы и проверка', 'summary': 'Создание, отправка на проверку, ручная оценка и возврат на доработку.', 'steps': [('Сформируйте работу', 'Выберите ученика, источник заданий и срок выполнения.'), ('Проверьте отправку', 'Откройте работу, просмотрите ответы и код по каждому заданию.'), ('Завершите проверку', 'Поставьте баллы, оставьте комментарии и выберите итоговый статус.')], 'links': [('Работы', '/assignments'), ('Очередь проверки', '/reviews/queue')]},
        {'slug': 'theory', 'icon': 'ph-book-open', 'title': 'Теория и материалы', 'summary': 'Создание теоретических блоков, публикация и связь с уроками.', 'steps': [('Создайте блок', 'Укажите тему, цель чтения и добавьте текст, формулы, код или файл.'), ('Проверьте просмотр', 'Используйте режим предпросмотра перед публикацией.'), ('Опубликуйте', 'После публикации блок становится доступен ученику в его теоретическом разделе.')], 'links': [('Библиотека', '/library')]},
        {'slug': 'analytics', 'icon': 'ph-chart-line-up', 'title': 'Аналитика ученика', 'summary': 'Как читать прогресс, слабые темы, навыки и сигналы внимания.', 'steps': [('Откройте аналитику', 'Смотрите показатели по проверенным работам, посещаемости и освоению тем.'), ('Найдите слабое место', 'Проверьте карту навыков и ошибки, затем добавьте повторение в курс.'), ('Сверьте динамику', 'Сравнивайте данные по периоду и меняйте маршрут только после фактической диагностики.')], 'links': [('Аналитика', '/analytics')]},
        {'slug': 'schedule', 'icon': 'ph-calendar-blank', 'title': 'Расписание и часовые пояса', 'summary': 'Планирование занятий, переносы и проверка времени у обеих ролей.', 'steps': [('Откройте календарь', 'Переключайте неделю и месяц, чтобы найти нужное занятие.'), ('Создайте или измените событие', 'Укажите ученика, дату, длительность, тип занятия и ссылку на комнату.'), ('Проверьте отображение', 'Сверьте время в часовом поясе преподавателя и ученика.')], 'links': [('Расписание', '/schedule')]},
        {'slug': 'profile', 'icon': 'ph-user-circle', 'title': 'Профиль и системные настройки', 'summary': 'Профиль создателя, преподавателя и персональные данные ученика.', 'steps': [('Откройте свой профиль', 'Используйте пункт профиля в текущей V2-навигации.'), ('Измените данные', 'Редактируйте только разрешённые поля и сохраните форму.'), ('Проверьте результат', 'Обновите страницу и убедитесь, что данные и изображения сохранились.')], 'links': [('Профиль', '/workspace/profile')]},
    ]
    student = [
        {'slug': 'dashboard', 'icon': 'ph-house', 'title': 'Главная и прогресс', 'summary': 'Где увидеть текущие работы, уроки, цели и ближайший шаг.', 'steps': [('Проверьте очередь', 'На главной отображаются активные работы и последние занятия.'), ('Откройте следующий шаг', 'Используйте карточку ближайшего урока или курса, чтобы продолжить обучение.'), ('Заполните профиль', 'Добавьте о себе, цели недели и аватар — данные сохраняются в вашем профиле.')], 'links': [('Моя главная', '/dashboard'), ('Профиль', '/workspace/profile')]},
        {'slug': 'courses', 'icon': 'ph-map-trifold', 'title': 'Программа обучения', 'summary': 'Как проходить курс, видеть модули и запускать назначенные уроки.', 'steps': [('Откройте программу', 'В разделе курса видны модули, уроки и текущий прогресс.'), ('Изучайте по маршруту', 'Начинайте с ближайшего элемента и отмечайте фактический статус.'), ('Войдите в урок', 'Нажмите «Войти в урок» у запланированного занятия — откроется единая комната.')], 'links': [('Главная ученика', '/dashboard')]},
        {'slug': 'lesson-room', 'icon': 'ph-video-camera', 'title': 'Комната урока', 'summary': 'Как пользоваться видео, заданием, доской, материалами и панелью урока.', 'steps': [('Подключитесь к видео', 'Нажмите кнопку видео и разрешите браузеру использовать камеру и микрофон.'), ('Следуйте этапу', 'Текущий этап и таймер видны в верхней части комнаты.'), ('Решайте задачу', 'Откройте практику, напишите код или ответ и сохраните результат.'), ('Дайте обратную связь', 'В панели урока выберите статус, уровень понимания и напишите, что повторить.')], 'links': [('Расписание', '/schedule')]},
        {'slug': 'submissions', 'icon': 'ph-clipboard-text', 'title': 'Задания и отправка', 'summary': 'Как открыть работу, сохранить ответ и отправить его преподавателю.', 'steps': [('Откройте работу', 'В списке заданий выберите нужную карточку и прочитайте условие.'), ('Сохраните решение', 'Заполните ответ или код и нажмите «Сохранить» — черновик можно продолжить позже.'), ('Отправьте на проверку', 'Когда всё готово, нажмите «Отправить». После проверки появятся баллы и комментарий преподавателя.')], 'links': [('Мои задания', '/submissions')]},
        {'slug': 'theory', 'icon': 'ph-book-open', 'title': 'Теория', 'summary': 'Как изучать опубликованные материалы и возвращаться к сложным темам.', 'steps': [('Выберите тему', 'Откройте опубликованный блок в теоретическом разделе.'), ('Работайте с материалом', 'Читайте текст, просматривайте код, формулы и прикреплённые файлы.'), ('Зафиксируйте вопрос', 'В комнате урока сообщите преподавателю, что нужно разобрать или повторить.')], 'links': [('Теория', '/theory')]},
        {'slug': 'analytics', 'icon': 'ph-chart-line-up', 'title': 'Моя аналитика', 'summary': 'Как увидеть динамику, карту навыков и прогноз подготовки.', 'steps': [('Откройте аналитику', 'Перейдите в отдельную страницу аналитики из навигации или профиля.'), ('Читайте карту навыков', 'Цвет и процент показывают фактическое освоение темы, а не случайный показатель.'), ('Выберите повторение', 'Слабые темы используйте как ориентир для следующей практики и вопросов преподавателю.')], 'links': [('Аналитика', '/analytics')]},
        {'slug': 'schedule', 'icon': 'ph-calendar-blank', 'title': 'Моё расписание', 'summary': 'Как найти занятие, проверить время и войти в комнату.', 'steps': [('Откройте календарь', 'Переключите нужную неделю или месяц.'), ('Проверьте время', 'Убедитесь, что часовой пояс профиля установлен верно.'), ('Войдите в урок', 'Откройте событие и перейдите в комнату урока.')], 'links': [('Расписание', '/schedule')]},
        {'slug': 'profile', 'icon': 'ph-user-circle', 'title': 'Мой профиль', 'summary': 'Заполнение личных данных, целей недели, часового пояса и аватара.', 'steps': [('Откройте профиль', 'Используйте пункт профиля в ученической V2-навигации.'), ('Заполните данные', 'Добавьте описание, цели и изображение без шаблонных значений.'), ('Сохраните и проверьте', 'После уведомления обновите страницу и убедитесь, что данные остались.')], 'links': [('Профиль', '/workspace/profile')]},
    ]
    items = common + (teacher if audience == 'teacher' else student)
    operational = {
        'start': {
            'before': ['Учётная запись BooStudy и активная сессия.', 'Понимание текущей роли: преподаватель или ученик.', 'Доступ к связанному ученику, курсу или уроку, если инструкция открыта из рабочего контекста.'],
            'checklist': ['Откройте нужный раздел через боковую навигацию.', 'Проверьте роль и имя контекста в шапке.', 'Выполните действие и дождитесь уведомления.', 'Обновите страницу и убедитесь, что результат сохранился.', 'Вернитесь по ссылке «Назад», а не через устаревший адрес.'],
            'result': 'После выполнения вы понимаете, где находится нужный инструмент, видите сохранённое состояние и можете продолжить сценарий с того же места.',
            'troubleshooting': ['Если раздел недоступен, проверьте активную роль и связь с учеником.', 'Если данные не обновились, повторите действие после полного ответа уведомления.', 'Если появился 403, не обходите его прямой ссылкой — вернитесь в V2-контекст, где выдан доступ.']
        },
        'students': {
            'before': ['Преподавательская роль или права создателя.', 'Связанный ученик либо готовая ссылка-приглашение.', 'Имя и контакт ученика для проверки приглашения.'],
            'checklist': ['Откройте «Ученики» и выберите нужный фильтр.', 'Создайте приглашение и скопируйте ссылку одним действием.', 'Попросите ученика открыть ссылку и завершить регистрацию.', 'Проверьте карточку: роль, связь, наставник, часовой пояс и активность.', 'Из карточки перейдите в дашборд, профиль, программу или расписание ученика.'],
            'result': 'Ученик появляется в списке с корректной связью; его карточка открывает только допустимые действия и актуальные V2-экраны.',
            'troubleshooting': ['При пустом списке проверьте фильтр «Все пользователи» и активную роль.', 'Если приглашение истекло, создайте новую ссылку, не редактируя старую запись.', 'Если карточка открывает чужой контекст, обновите сессию имперсонации и повторите переход.']
        },
        'courses': {
            'before': ['Связанный ученик.', 'Выбранная экзаменационная программа.', 'Цель, дата экзамена и исходный уровень ученика.'],
            'checklist': ['Создайте курс и заполните цель, ожидаемый результат и длительность занятия.', 'Добавьте модули и уроки в нужном порядке.', 'Откройте «Настроить навыки» и добавьте темы, номера заданий и зависимости.', 'Запустите мастер маршрута: частота, нагрузка, диагностика и дата экзамена.', 'Проверьте недельный план, предупреждения prerequisite и прогноз.', 'Запустите конкретный урок кнопкой «Начать занятие».'],
            'result': 'Курс имеет версию маршрута, навыки, модули, уроки и реальные LearningItem; запланированный урок открывается в комнате V2.',
            'troubleshooting': ['Если генератор сообщает, что навыков нет, сначала добавьте их в карте навыков.', 'Если урок не запускается, проверьте, что он принадлежит текущему курсу и имеет статус planned.', 'Если прогноз показывает диапазон, учитывайте неопределённость — это не фиксированная оценка.']
        },
        'lesson-room': {
            'before': ['Запланированный или активный урок.', 'Ученик подключён к курсу.', 'Для видео — разрешения браузера на камеру и микрофон.'],
            'checklist': ['Откройте комнату и проверьте имя ученика, название урока и статус синхронизации.', 'Запустите Daily-видео и убедитесь, что оба участника видят подключение.', 'Переключайтесь между практикой, теорией, доской, материалами и заданиями.', 'В панели урока меняйте этап, таймер, статус ученика, понимание и заметки.', 'Сохраняйте код, файлы, доску и комментарии по мере работы.', 'В конце зафиксируйте итог урока и следующий шаг.'],
            'result': 'Состояние комнаты синхронизировано у преподавателя и ученика, а результат урока сохранён в истории курса и аналитике.',
            'troubleshooting': ['Если вкладка не открывается, проверьте параметр pane и вернитесь через верхнюю панель комнаты.', 'Если Daily возвращает 500, проверьте переменные Daily на сервере и не создавайте новый room вручную.', 'Если панель уезжает вниз, используйте закреплённую боковую панель и не меняйте масштаб страницы.']
        },
        'assignments': {
            'before': ['Ученик и курс.', 'Источник задач: генератор, шаблон или ручные ID.', 'Срок и тип выдачи.'],
            'checklist': ['Создайте работу из V2-конструктора.', 'Выберите учеников и источник задач.', 'Проверьте карточки заданий перед публикацией.', 'Назначьте срок, режим проверки и дополнительные материалы.', 'Откройте отправку ученика и проверьте каждый ответ или файл.', 'Поставьте баллы, комментарии и итоговый статус; при необходимости верните на доработку.'],
            'result': 'Работа появляется у ученика, отправляется в очередь проверки, а завершённая проверка учитывается в прогрессе и аналитике.',
            'troubleshooting': ['Если генератор пуст, настройте навыки курса и проверьте программу.', 'Если отправка не видна, откройте правильный курс и фильтр статуса.', 'Не выставляйте итоговый статус до сохранения баллов и комментария.']
        },
        'theory': {
            'before': ['Преподаватель: тема и материалы; ученик: опубликованный блок.', 'Файл должен быть доступен хранилищу и соответствовать допустимому типу.'],
            'checklist': ['Создайте теоретический блок с названием, целью и временем чтения.', 'Добавьте текст, код, формулы, изображения или файл.', 'Сохраните черновик и откройте предпросмотр.', 'Свяжите блок с курсом или уроком.', 'Опубликуйте блок и проверьте его в ученической роли.', 'Используйте отметки изучения и обратную связь ученика для следующего повторения.'],
            'result': 'Ученик видит опубликованный материал в своей V2-теории, а прогресс чтения и обратная связь доступны преподавателю.',
            'troubleshooting': ['Если ученик не видит блок, проверьте статус публикации и связь с его курсом.', 'Если предпросмотр пуст, сохраните черновик и проверьте прикреплённый файл.', 'Не используйте старый маршрут библиотеки — открывайте раздел из V2-навигации.']
        },
        'analytics': {
            'before': ['Есть проверенные работы, завершённые уроки или диагностические ответы.', 'Для прогноза заполнены цель и экзаменационная программа.'],
            'checklist': ['Откройте отдельную страницу аналитики из навигации.', 'Сверьте KPI: выполнение, средний балл, пробники и посещаемость.', 'Изучите динамику по периоду и карту навыков.', 'Откройте слабую тему и проверьте ошибки или prerequisite.', 'Добавьте повторение или новый элемент в маршрут.', 'После следующей проверки сравните изменение метрик.'],
            'result': 'Метрики строятся по фактическим данным пользователя; решения по маршруту можно связать с конкретной слабой темой и источником сигнала.',
            'troubleshooting': ['Нули означают отсутствие проверенных данных, а не случайно заданный результат.', 'Если карта навыков пуста, сначала свяжите курс с навыками и завершите диагностику.', 'Прогноз всегда рассматривайте вместе с диапазоном и пояснением неопределённости.']
        },
        'dashboard': {
            'before': ['Активная ученическая сессия.', 'Связанный преподаватель или курс — если ожидаются занятия.'],
            'checklist': ['Проверьте верхнюю карточку и текущий прогресс.', 'Откройте ближайший урок или программу обучения.', 'Просмотрите очередь работ и последние занятия.', 'Добавьте личную цель недели и заполните профиль только своими данными.', 'Перейдите в аналитику, если нужен детальный разбор навыков.'],
            'result': 'Главная показывает только реальные данные текущего ученика, а каждый блок ведёт в соответствующий V2-сценарий.',
            'troubleshooting': ['Пустой список означает, что преподаватель ещё не назначил работу или урок.', 'Если отображается чужое имя, завершите имперсонацию и войдите под нужной учётной записью.', 'Случайные стрики и посещения не являются нормальным состоянием — проверьте историю активности.']
        },
        'submissions': {
            'before': ['Работа назначена преподавателем.', 'Открыт доступ к условию и редактору ответа.'],
            'checklist': ['Откройте нужную работу из дашборда или комнаты урока.', 'Выберите задание и прочитайте условие до начала решения.', 'Введите ответ, код или прикрепите файл.', 'Сохраните черновик и проверьте, что версия появилась в истории.', 'Нажмите «Отправить на проверку» только после финальной проверки.', 'После комментария преподавателя исправьте решение или подтвердите результат.'],
            'result': 'Работа имеет понятный статус: черновик, отправлена, на проверке, проверена или возвращена; баллы и комментарии сохраняются.',
            'troubleshooting': ['Если кнопка отправки недоступна, сначала сохраните ответ.', 'Если файл не открывается, проверьте размер и тип, затем повторите загрузку.', 'Возвращённую работу нельзя считать завершённой — исправьте её и отправьте снова.']
        },
        'profile': {
            'before': ['Авторизованная сессия владельца профиля.', 'Изображение и текст, которые вы действительно хотите сохранить.'],
            'checklist': ['Откройте «Профиль» из своей навигации.', 'Измените имя, описание, часовой пояс или цели.', 'Загрузите аватар и обложку; настройте кадрирование и положение.', 'Сохраните изменения и дождитесь уведомления на этом же экране.', 'Обновите страницу и проверьте, что данные не вернулись к прежним значениям.'],
            'result': 'Профиль показывает только введённые пользователем данные; системные метрики считаются из истории, а не из шаблонных чисел.',
            'troubleshooting': ['Если изображение не отображается, проверьте URL загрузки и наличие файла в хранилище.', 'Если часовой пояс не сохраняется, повторите сохранение после выбора значения из списка.', 'Создательский системный профиль нельзя менять через ученический экран.']
        },
        'schedule': {
            'before': ['Преподаватель создал занятие или ученик связан с курсом.', 'Часовой пояс профиля задан корректно.'],
            'checklist': ['Откройте календарь и переключите неделю или месяц.', 'Проверьте дату, время и часовой пояс.', 'Создайте или откройте занятие.', 'Перейдите из события прямо в комнату урока.', 'После изменения времени обновите календарь и проверьте обе роли.'],
            'result': 'Расписание отображает фактические занятия в часовом поясе пользователя, а событие ведёт в актуальную комнату без старого маршрута.',
            'troubleshooting': ['Если время смещено, сначала исправьте часовой пояс в профиле.', 'Если событие не видно ученику, проверьте его связь с курсом и статус публикации.', 'Не создавайте дубликат, пока не обновили календарь после сохранения.']
        }
        ,
        'telegram': {
            'before': ['Авторизованный профиль BooStudy.', 'Установлен Telegram на устройстве.', 'Доступ к чату нужного BooStudy-бота.'],
            'checklist': ['Сгенерируйте одноразовый код в профиле.', 'Откройте ссылку на бота и нажмите Start.', 'Передайте код боту и дождитесь подтверждения.', 'Вернитесь в BooStudy и обновите карточку интеграции.', 'Откройте Mini App и проверьте расписание или доступную роль.', 'Проверьте, что напоминание приходит в нужный чат.'],
            'result': 'Telegram связан с правильным профилем, а уведомления и Mini App используют его часовой пояс и права доступа.',
            'troubleshooting': ['Код одноразовый: при истечении срока сгенерируйте новый.', 'Если статус не изменился, убедитесь, что код отправлен тому же боту, ссылка на которого показана в профиле.', 'Если уведомление не пришло, проверьте, что чат не заблокирован и связь отображается как активная.']
        }
    }
    system_explanations = {
        'start': ('Навигация и контекст', 'Платформа строит интерфейс по активной роли и текущему контексту ученика, курса или урока. Сначала выбирается контекст, затем действие записывается в его источник данных.', 'Пользователь; активная роль; текущий объект.', 'Смена роли меняет доступные разделы, а сохранённый результат остаётся в PostgreSQL и доступен связанным экранам.'),
        'students': ('Связь «преподаватель — ученик»', 'Приглашение создаёт безопасную ссылку, регистрация создаёт пользователя, а связь наставника определяет, какие курсы, уроки и метрики видит каждая сторона.', 'Преподаватель; ученик; родитель — только в разрешённом read-only-контексте.', 'Изменение связи сразу влияет на списки учеников, доступ к курсам и видимость комнаты.'),
        'courses': ('Курс как маршрут', 'Курс состоит из версии маршрута, модулей, уроков, навыков и LearningItem. Навыки задают содержание, генератор строит последовательность, а запуск урока переводит его в Studio.', 'Преподаватель создаёт и меняет; ученик проходит; родитель читает сводку.', 'Старт, перенос и завершение уроков меняют план недели, прогресс, ошибки и прогноз.'),
        'lesson-room': ('Комната как единое рабочее пространство', 'Комната объединяет Daily-видео и синхронизированные вкладки: практика, теория, доска, материалы, задания и панель урока. Преподаватель управляет этапом, ученик отправляет состояние и ответы.', 'Преподаватель — ведущий; ученик — участник; сервер — источник синхронизации.', 'Состояния этапов, таймеры, доска, код и итог урока сохраняются в контексте Studio.'),
        'assignments': ('Жизненный цикл работы', 'Работа связывает ученика, набор заданий, срок и правила проверки. Ответ проходит состояния черновик → отправлено → проверка → проверено или доработка.', 'Преподаватель — автор и проверяющий; ученик — исполнитель.', 'Баллы и комментарии обновляют прогресс, навыки, ошибки и аналитические показатели.'),
        'theory': ('Публикация знаний', 'Теория хранится как блоки контента с черновиком, предпросмотром и публикацией. Ученик получает только опубликованные блоки, связанные с его курсом или уроком.', 'Преподаватель — редактор; ученик — читатель и источник обратной связи.', 'Изучение и вопросы влияют на обратную связь урока и будущие повторения.'),
        'analytics': ('Метрики из фактов', 'Аналитика не хранит случайные цифры: она агрегирует проверки, посещения, завершённые уроки, StudentSkill и историю ошибок.', 'Ученик видит личные метрики; преподаватель — расширенный разбор ученика.', 'Новая проверка или завершённый урок меняют KPI, карту навыков, прогноз и сигналы внимания.'),
        'dashboard': ('Операционный центр', 'Дашборд собирает ближайшие действия из курсов, работ, расписания и уведомлений. Он не создаёт фиктивную активность, а показывает состояние связанных сущностей.', 'Ученик; преподаватель видит собственные рабочие сводки.', 'Из карточек можно перейти в первоисточник: курс, урок, работу, профиль или аналитику.'),
        'submissions': ('Ответ ученика и проверка', 'Редактор хранит черновики и версии ответа, а отправка создаёт проверяемую запись. Преподаватель оценивает именно отправленную версию, а не локальное содержимое поля.', 'Ученик — автор; преподаватель — проверяющий.', 'После проверки статус и баллы становятся частью истории задания и аналитики.'),
        'profile': ('Профиль и настройки', 'Профиль хранит личные поля, изображения, часовой пояс и цели. Системные показатели — стрик, посещения, XP — рассчитываются отдельно из событий и не редактируются формой профиля.', 'Владелец профиля; создатель — защищённый системный профиль.', 'Часовой пояс влияет на расписание и Telegram, описание и цели отображаются в личных экранах.'),
        'schedule': ('Расписание как события', 'Календарь показывает события занятий в часовом поясе пользователя. Событие связывает ученика, преподавателя, курс и комнату урока.', 'Преподаватель создаёт и переносит; ученик видит и входит.', 'Изменение события меняет ближайшие занятия на дашборде и напоминания Telegram.'),
        'telegram': ('Внешний канал уведомлений', 'Профиль генерирует одноразовый код, бот связывает Telegram chat с пользователем, а уведомления используют сохранённую связь и часовой пояс.', 'Пользователь; BooStudy web; Telegram-бот; Mini App.', 'Связь не дублирует данные платформы: она доставляет события расписания, дедлайнов и доступные действия.')
    }
    component_maps = {
        'courses': [
            ('Курс', 'Корневая сущность ученика: цель, программа, ожидаемый результат, дата экзамена и статус.'),
            ('Версия маршрута', 'Неизменяемый снимок структуры курса; новая правка создаёт следующую версию и позволяет сравнить изменения.'),
            ('Модули', 'Логические блоки программы, упорядочивающие уроки и связывающие их с результатом обучения.'),
            ('Уроки', 'Запланированные или проведённые занятия с порядком, датой, длительностью, статусом и входом в Studio.'),
            ('ExamSkill', 'Атомарная тема экзамена: код, раздел, номера задач, сложность и prerequisite-зависимости.'),
            ('StudentSkill', 'Персональное освоение навыка: процент, уровень уверенности, последняя диагностика, ошибки и дата повторения.'),
            ('LearningItem', 'Конкретный элемент маршрута: теория, практика, ДЗ, контроль или повторение с дедлайном и причиной появления.'),
            ('Мастер программы', 'Форма цели, частоты, длительности, нагрузки ДЗ, режима диагностики и стартового прогноза.'),
            ('Генератор', 'Подбирает элементы по цели, навыкам, prerequisite, прогнозу и доступному времени; предупреждения объясняют спорные места.'),
            ('Недельный план и центр внимания', 'Показывают просрочки, обязательные повторения, ошибки и незавершённые уроки, чтобы менять маршрут по фактам.'),
            ('Запуск и результат', 'Старт переводит урок в Studio и переносит agenda; завершение создаёт LessonOutcome, обновляет навыки, ошибки и аналитику.')
        ],
        'students': [('Список', 'Фильтры и карточки пользователей.'), ('Приглашение', 'Одноразовая ссылка регистрации.'), ('Связь', 'Наставник и права доступа.'), ('Контекст', 'Переход к профилю, дашборду, курсу и расписанию.'), ('События', 'Изменения связи влияют на видимость данных.')],
        'lesson-room': [('Шапка', 'Ученик, урок, синхронизация и этап.'), ('Daily', 'Видео и аудио-сессия участников.'), ('Пространства', 'Практика, теория, доска, материалы и задания.'), ('Панель урока', 'Этап, таймер, маршрут, заметки, статус и понимание.'), ('Realtime', 'Серверное состояние комнаты и события синхронизации.'), ('Outcome', 'Итог урока и следующий шаг.')],
        'assignments': [('Работа', 'Ученик, тип, срок и правила.'), ('Источник', 'ID, генератор, шаблон или быстрый пробник.'), ('Submission', 'Версии ответа и вложений ученика.'), ('Review', 'Баллы, комментарии, статус и доработка.'), ('Метрики', 'Навыки, ошибки, прогресс и аналитика.')],
        'theory': [('Блок', 'Тема, цель, время чтения и статус.'), ('Редактор', 'Текст, код, формулы, изображения и файлы.'), ('Предпросмотр', 'Проверка отображения до публикации.'), ('Публикация', 'Доступ ученику через курс или урок.'), ('Learning cycle', 'Изучение, отметка, обратная связь и повторение.')],
        'analytics': [('Факты', 'Проверки, посещения, уроки и ответы.'), ('KPI', 'Выполнение, средняя оценка, пробники и посещаемость.'), ('Динамика', 'Изменение результата за период.'), ('Карта навыков', 'StudentSkill по темам и prerequisite.'), ('Прогноз', 'Диапазон результата с объяснением неопределённости.'), ('Сигналы', 'Ошибки, повторения и точки внимания.')],
        'profile': [('Identity', 'Имя, логин, роль и доступ.'), ('Контент', 'О себе, цели и личные настройки.'), ('Изображения', 'Аватар и обложка с кадрированием.'), ('Timezone', 'Часовой пояс для расписания и Telegram.'), ('Derived metrics', 'Стрик, XP и посещаемость считаются из событий.')],
        'schedule': [('Event', 'Ученик, преподаватель, курс и время.'), ('Timezone', 'Преобразование времени под пользователя.'), ('Calendar', 'Неделя, месяц и фильтры.'), ('Room link', 'Переход в конкретный урок.'), ('Reminders', 'Telegram-события по опубликованному занятию.')],
        'telegram': [('Auth code', 'Одноразовый код из профиля.'), ('Bot link', 'Привязка Telegram chat к User.'), ('Mini App', 'Расписание и доступные действия.'), ('Digest', 'Напоминания и дедлайны.'), ('Unlink', 'Безопасное отключение связи.')],
        'dashboard': [('Summary', 'Сводка текущего состояния.'), ('Queue', 'Активные работы и ближайшие действия.'), ('History', 'Последние уроки и события.'), ('Trajectory', 'Слабые темы и маршрут.'), ('Shortcuts', 'Переход к первоисточнику данных.')],
        'submissions': [('Assignment', 'Условие и критерии.'), ('Editor', 'Ответ, код или файл.'), ('Drafts', 'Сохранённые промежуточные версии.'), ('Submit', 'Фиксация версии для проверки.'), ('Feedback', 'Баллы, комментарии и повторная отправка.')],
        'start': [('Session', 'Авторизация и активная роль.'), ('Navigation', 'V2-навигация по доступным разделам.'), ('Context', 'Текущий ученик, курс или урок.'), ('Action', 'Изменение конкретной сущности.'), ('Notification', 'Подтверждение результата на том же экране.')]
    }
    deep_details = {
        'courses': {
            'workflow': [
                ('1. Контекст ученика', 'Преподаватель открывает конкретного ученика и выбирает экзаменационную программу. Курс всегда принадлежит этому ученику, поэтому все последующие метрики и уроки не смешиваются с чужими данными.'),
                ('2. Цель и ограничения', 'В мастере задаются цель/балл, дата экзамена, частота занятий, длительность урока, допустимая нагрузка домашних работ и режим диагностики. Эти параметры становятся входом генератора, а не декоративными полями.'),
                ('3. Карта навыков', 'К курсу привязываются ExamSkill с кодом, номерами заданий, сложностью и prerequisite. После диагностики у ученика появляются StudentSkill с процентом освоения, уверенностью, ошибками и датой следующего повторения.'),
                ('4. Построение версии', 'Генератор подбирает LearningItem и раскладывает их по модулям и неделям. Каждое изменение создаёт новую версию маршрута: опубликованная версия остаётся снимком для истории, черновик можно пересобрать и сравнить.'),
                ('5. Проверка перед публикацией', 'Мастер показывает предупреждения: нет навыков, разорваны prerequisite, нет времени до экзамена, перегружена неделя или отсутствуют материалы. Публикация доступна только после устранения блокирующих предупреждений.'),
                ('6. Запуск занятия', 'Урок из маршрута получает дату и статус, а кнопка запуска открывает каноничную Studio-комнату с тем же course/module/lesson-контекстом. Ученик видит только назначенные ему элементы.'),
                ('7. Факт вместо плана', 'Завершение урока создаёт LessonOutcome: фактическое время, освоенные навыки, ошибки, комментарий и следующий шаг. Эти факты обновляют недельный план, центр внимания, аналитику и прогноз.'),
                ('8. Пересборка', 'Если диагностика или проверка выявила слабую тему, преподаватель не исправляет цифру вручную: он добавляет повторение или меняет навык. Следующая версия маршрута объясняет, почему элемент появился и что именно он закрывает.')
            ],
            'states': [
                ('Курс', 'draft → active → paused → completed/archived', 'В archived нельзя создавать новые уроки, история остаётся доступной.'),
                ('Версия маршрута', 'draft → review → published → superseded', 'Одновременно для курса действует одна published-версия.'),
                ('LearningItem', 'planned → in_progress → completed → needs_review', 'Статус меняется по факту урока, проверки или отметки ученика.'),
                ('Урок', 'scheduled → live → completed → cancelled', 'live открывает Studio, completed создаёт outcome.'),
                ('Навык', 'unknown → emerging → practiced → mastered', 'Переход рассчитывается из проверенных фактов и повторений.')
            ],
            'permissions': ['Преподаватель создаёт курс, навыки, версии, модули, уроки и назначения.', 'Ученик читает опубликованный маршрут, проходит LearningItem и видит собственный прогресс.', 'Создатель и администратор видят полный контекст для поддержки, но не подменяют факты ученика.', 'Родитель получает только разрешённую сводку без редактора и внутренних комментариев.'],
            'persistence': ['Course и его версия сохраняются в PostgreSQL.', 'Модули, уроки и LearningItem имеют ссылки на курс и порядок внутри версии.', 'StudentSkill, LessonOutcome и проверки являются источниками прогресса; KPI не вводятся вручную.', 'Каждый переход статуса должен подтверждаться уведомлением и переживать обновление страницы.', 'Импорт/экспорт должен переносить идентификаторы навыков, порядок, статусы публикации и связи prerequisite.'],
            'integrations': ['Генератор использует ExamSkill, StudentSkill, prerequisite, дату экзамена и доступное время.', 'Studio получает lesson_id, course_id и активный этап; результат возвращается в курс.', 'Расписание создаёт вход в конкретный урок, а Telegram отправляет напоминание по часовому поясу пользователя.', 'Аналитика читает completed submissions, посещения, outcomes и историю навыков.'],
            'checks': ['Создать курс без навыков: интерфейс показывает блокирующее предупреждение.', 'Собрать маршрут, опубликовать версию и обновить страницу: версия остаётся опубликованной.', 'Запустить урок из карточки курса в роли преподавателя и войти учеником: открывается одна Studio-комната.', 'Завершить урок с ошибкой по навыку: в центре внимания появляется повторение.', 'Создать новую версию: старая не меняется задним числом, а ученик видит опубликованную версию.']
        },
        'lesson-room': {
            'workflow': [('Контекст', 'Урок открывается с учеником, курсом и текущим этапом.'), ('Синхронизация', 'Сервер хранит этап, таймер, активную вкладку и состояние доски.'), ('Работа', 'Участники используют Daily, практику, теорию, доску, материалы и задания.'), ('Итог', 'Панель фиксирует статус ученика, понимание, заметки и следующий шаг.')],
            'states': [('Этап', 'preparation → practice → summary', 'Переход ведущего виден ученику.'), ('Таймер', 'idle → running → paused → finished', 'Для каждого этапа хранится своё оставшееся время.'), ('Комната', 'scheduled → live → completed', 'Закрытие создаёт LessonOutcome.')],
            'permissions': ['Преподаватель управляет этапом, таймером, доской и принудительной синхронизацией.', 'Ученик меняет собственный статус и отправляет саморефлексию.', 'Оба участника видят только данные текущей комнаты.'],
            'persistence': ['Realtime-события восстанавливаются после обновления страницы.', 'Код, материалы, доска и панель используют lesson_id как ключ.', 'Финальный outcome является источником для курса и аналитики.'],
            'integrations': ['Daily для видео; Socket.IO для синхронизации; хранилище для материалов; проверка для отправки кода.'],
            'checks': ['Открыть две роли, переключить этап и обновить обе страницы.', 'Поставить паузу ученику и проверить статус у преподавателя.', 'Нарисовать, открыть материалы и вернуться на доску без потери состояния.']
        },
        'assignments': {
            'workflow': [('Конструктор', 'Автор выбирает учеников, тип работы, источник задач, дедлайн и правила проверки.'), ('Пул', 'Генератор, шаблон или ручные ID формируют карточки; до публикации их можно заменить и проверить.'), ('Выдача', 'Публикация создаёт assignment для каждого ученика и показывает работу в его очереди.'), ('Отправка', 'Ученик сохраняет версии ответа, прикладывает файлы и фиксирует одну версию отправкой.'), ('Проверка', 'Преподаватель открывает именно отправленную версию, ставит баллы по заданиям, комментарий и статус.'), ('Цикл доработки', 'Статус needs_revision возвращает работу ученику; новая отправка сохраняет историю, не затирая предыдущую.')],
            'states': [('Работа', 'draft → published → in_progress → submitted → reviewing → graded/needs_revision', 'Статус виден обеим ролям.'), ('Версия', 'draft → submitted → reviewed', 'Оценка привязана к submitted-версии.'), ('Задание', 'pending → answered → checked', 'Баллы учитываются только после проверки.')],
            'permissions': ['Преподаватель создаёт и проверяет работы связанных учеников.', 'Ученик редактирует только свой draft и не меняет итоговую оценку.', 'Создатель и администратор видят очередь для поддержки и аудита.'],
            'persistence': ['Assignment, task cards, submission versions, review rows и вложения связаны по ID.', 'Баллы и комментарии записываются до смены итогового статуса.', 'История отправок не удаляется при возврате на доработку.'],
            'integrations': ['Генератор и библиотека дают источник задач; курс даёт навыки; проверка обновляет аналитику; Telegram уведомляет о дедлайне и результате.'],
            'checks': ['Создать draft и обновить страницу.', 'Отправить работу учеником и убедиться, что она появилась в очереди преподавателя.', 'Проверить балл/комментарий, вернуть на доработку и повторно отправить без потери истории.']
        },
        'theory': {
            'workflow': [('Автор', 'Преподаватель создаёт блок, выбирает группу, время чтения и добавляет текст, код, формулы, файлы и мини-проверки.'), ('Черновик', 'Сохранение не публикует материал: его можно открыть в предпросмотре и исправить.'), ('Публикация', 'Опубликованный блок появляется только у учеников с доступом к курсу/назначению.'), ('Изучение', 'Ученик читает, отмечает прогресс, сохраняет закладку и личную заметку.'), ('Обратная связь', 'Checkpoint фиксирует попытки и правильность, feedback сообщает преподавателю уровень понимания.'), ('Повторение', 'Низкий результат или назначение преподавателя создаёт следующий учебный шаг.')],
            'states': [('Блок', 'draft → published → archived', 'Видимость ученика начинается с published.'), ('Чтение', 'unopened → in_progress → read', 'Прогресс и позиция переживают обновление.'), ('Проверка', 'unanswered → correct/incorrect', 'Попытки и объяснение сохраняются.')],
            'permissions': ['Преподаватель редактирует свои блоки и назначает их ученикам.', 'Ученик не меняет содержание, но пишет заметку, отвечает на checkpoint и оставляет feedback.', 'Администратор видит контент для аудита.'],
            'persistence': ['TheoryBlock хранит содержание и статус публикации.', 'StudentTheoryState хранит процент, позицию, bookmark и is_read.', 'CheckpointAttempt, Note и Feedback сохраняются отдельными связанными записями.'],
            'integrations': ['Курс и LearningItem определяют доступ; комната показывает теорию; аналитика читает результат; файлы идут через общее хранилище.'],
            'checks': ['Сохранить черновик и убедиться, что ученик его не видит.', 'Опубликовать, пройти блок учеником и обновить страницу.', 'Ответить на checkpoint неверно/верно и проверить попытки и feedback преподавателя.']
        },
        'analytics': {
            'workflow': [('Сбор фактов', 'Система получает проверки, отправки, посещения, уроки, StudentSkill и ошибки.'), ('Агрегация', 'KPI и карта навыков пересчитываются из фактов за выбранный период.'), ('Интерпретация', 'Слабая тема связывается с ошибкой, prerequisite и конкретным LearningItem.'), ('Действие', 'Преподаватель добавляет повторение или меняет маршрут; ученик выбирает следующий шаг.'), ('Проверка эффекта', 'Следующая работа сравнивается с предыдущей динамикой.')],
            'states': [('Метрика', 'no_data → partial → reliable', 'Нули означают отсутствие фактов.'), ('Навык', 'unknown → emerging → practiced → mastered', 'Переход основан на проверенных ответах.'), ('Прогноз', 'calculated → recalculated', 'Меняется после новых фактов.')],
            'permissions': ['Ученик видит свои KPI и прогноз.', 'Преподаватель видит детализацию связанного ученика.', 'Администратор видит агрегаты для контроля системы.'],
            'persistence': ['Первичны события и оценки; визуальные KPI строятся при запросе.', 'Период фильтра не меняет исходные факты.', 'Каждый сигнал содержит ссылку на источник, если он доступен.'],
            'integrations': ['Проверка, курс, комната, расписание и теория поставляют факты; дашборд показывает краткую выжимку.'],
            'checks': ['Проверить пустой профиль: нет случайных чисел.', 'Завершить проверку и увидеть изменение KPI.', 'Открыть слабую тему и перейти к первоисточнику задания или навыка.']
        },
        'profile': {
            'workflow': [('Identity', 'Владелец меняет разрешённые личные поля, не затрагивая роль и системный ID.'), ('Контент', 'О себе и цели недели сохраняются как личные данные.'), ('Медиа', 'Аватар/обложка проходят загрузку и кадрирование, после чего получают постоянный URL.'), ('Timezone', 'Выбранный IANA-пояс сохраняется и используется календарём и Telegram.'), ('Проверка', 'После ответа сервера экран показывает уведомление, обновление страницы подтверждает сохранение.')],
            'states': [('Профиль', 'incomplete → onboarding_done', 'Гайд показывается только до завершения первичного заполнения.'), ('Изображение', 'empty → uploading → ready', 'Ошибка не должна подменяться случайной аватаркой.'), ('Цель', 'planned → active → completed', 'Цель принадлежит текущему ученику.')],
            'permissions': ['Владелец редактирует свои данные.', 'Создательский профиль защищён от обычного редактирования.', 'Преподаватель видит только разрешённую карточку ученика.'],
            'persistence': ['User/Profile и Student хранят личные поля отдельно от derived metrics.', 'Стрик, XP и посещения считаются из событий.', 'Файлы лежат в хранилище, URL хранится в профиле.'],
            'integrations': ['Timezone используется расписанием и Telegram; цели читаются дашбордом; аватар — навигацией и карточками.'],
            'checks': ['Создать нового ученика и убедиться в пустом профиле без хардкода.', 'Изменить timezone, обновить страницу и проверить календарь.', 'Загрузить изображение и открыть профиль из другой роли.']
        },
        'schedule': {
            'workflow': [('Создание', 'Преподаватель выбирает ученика, курс, дату, длительность и тип занятия.'), ('Публикация', 'Событие появляется в календаре обеих ролей и получает ссылку в комнату.'), ('Перенос', 'Изменяется одно событие, а не создаётся дубликат.'), ('Напоминание', 'Telegram отправляет уведомление с локальным временем получателя.'), ('Вход', 'Кнопка события открывает конкретный lesson_id.')],
            'states': [('Событие', 'draft → scheduled → live → completed/cancelled', 'Статус определяет доступ к комнате.'), ('Напоминание', 'pending → sent/failed', 'Ошибка доставки не меняет событие.')],
            'permissions': ['Преподаватель создаёт и переносит занятия.', 'Ученик видит свои события и входит в комнату.', 'Родитель видит разрешённую сводку.'],
            'persistence': ['Дата хранится с timezone-aware значением; отображение преобразуется в пояс пользователя.', 'Событие связано с lesson/course/student/teacher.', 'Перенос сохраняет историю и обновляет напоминание.'],
            'integrations': ['Курс даёт урок; комната принимает lesson_id; дашборд показывает ближайшее событие; Telegram доставляет напоминание.'],
            'checks': ['Создать событие в одном поясе и сверить у ученика.', 'Перенести занятие и убедиться, что старый слот исчез.', 'Войти по ссылке из календаря и проверить тот же урок.']
        }
    }
    generic_deep = {
        'workflow': [('Открытие', 'Система загружает только доступный текущей роли контекст и показывает его в шапке.'), ('Изменение', 'Пользователь выполняет действие в форме или карточке, после чего сервер проверяет права и входные данные.'), ('Сохранение', 'Изменение записывается в PostgreSQL, а уведомление появляется на том же экране.'), ('Продолжение', 'Связанные V2-экраны читают тот же источник данных, поэтому результат виден без ручного дублирования.')],
        'states': [('Данные', 'empty → draft → active → completed', 'Пустое состояние не является хардкодом и показывает отсутствие факта.'), ('Доступ', 'allowed → denied', '403 означает отсутствие права или неверный контекст, а не повод использовать legacy-маршрут.')],
        'permissions': ['Пользователь изменяет только принадлежащие ему поля.', 'Преподаватель видит связанные данные ученика в рамках учебного контекста.', 'Создатель и администратор используют служебные операции по RBAC.'],
        'persistence': ['Источник данных и идентификатор записи сохраняются на сервере.', 'Обновление страницы не должно возвращать форму к шаблонным значениям.', 'Производные показатели пересчитываются из событий, а не вводятся вручную.'],
        'integrations': ['Связанные разделы получают изменения через каноничные V2-ссылки.', 'Telegram используется только для уведомлений, если раздел с ним связан.'],
        'checks': ['Выполнить действие и обновить страницу.', 'Проверить результат второй ролью.', 'Открыть пустое состояние и убедиться, что нет фиктивных чисел.']
    }
    for item in items:
        detail = operational.get(item['slug'], operational['start'])
        item.update(detail)
        explanation = system_explanations.get(item['slug'], system_explanations['start'])
        item['system_title'], item['architecture'], item['participants'], item['data_effect'] = explanation
        item['components'] = component_maps.get(item['slug'], component_maps['start'])
        detail = deep_details.get(item['slug'], generic_deep)
        item.update(detail)
        item['audience'] = audience
    return items


@main_bp.route('/guidebook')
@login_required
def guidebook():
    role = 'teacher' if get_active_role() in {'creator', 'admin', 'tutor', 'teacher'} else 'student'
    requested = request.args.get('role')
    if requested in {'teacher', 'student'} and (role == requested or current_user.is_creator() or current_user.is_admin()):
        role = requested
    return render_template('guidebook.html', audience=role, sections=_guidebook_sections(role))


@main_bp.route('/guidebook/<audience>/<slug>')
@login_required
def guidebook_detail(audience, slug):
    if audience not in {'teacher', 'student'}:
        abort(404)
    active_role = get_active_role()
    can_view = audience == 'student' or active_role in {'creator', 'admin', 'tutor', 'teacher'} or current_user.is_creator() or current_user.is_admin()
    if not can_view:
        abort(403)
    sections = _guidebook_sections(audience)
    section = next((item for item in sections if item['slug'] == slug), None)
    if section is None:
        abort(404)
    return render_template('guidebook_detail.html', audience=audience, section=section, sections=sections)
...
def _require_dev_tools(*, administrator_only=False):
    ...
    # Локальная/тестовая среда не расширяет права: переключатель ролей сам
    # является привилегированной операцией и доступен только staff/QA-сессиям.
...
def dev_get_users_api():
    ...
    # В изолированном pytest схема создаётся уже после app bootstrap, поэтому
    # системная учётная запись creator должна быть добавлена только здесь.
    # Локальный и production-контуры не выполняют запись на GET.
    if current_app.config.get('TESTING') and User.query.filter_by(username='creator').first() is None:
    ...
    # Pytest создаёт собственные изолированные роли. В остальных окружениях
    # переключатель показывает стабильный каноничный набор уже существующих
    # аккаунтов и никогда не создаёт пользователей по GET-запросу.
    ...
    # Загружаем только реальные активные аккаунты в заданном порядке.
...
def parents_dashboard():
    ...
    from app.models import LearningTrajectory, LearningItem, StudentSkill
    ...
    for tie in children_ties:
        ...
        if child_student:
            ...
            active_courses = LearningTrajectory.query.filter_by(student_id=child_student.student_id, status='active').order_by(LearningTrajectory.updated_at.desc()).all()
            active_course = active_courses[0] if active_courses else None
            course_mastery = 0
            course_next_lesson = None
            course_cards = []
            if active_course:
                for course_row in active_courses:
                    course_items = LearningItem.query.filter_by(course_id=course_row.course_id).all()
                    skill_ids = [item.skill_id for item in course_items if item.skill_id]
                    skill_rows = StudentSkill.query.filter(StudentSkill.student_id == child_student.student_id, StudentSkill.skill_id.in_(skill_ids)).all() if skill_ids else []
                    mastery = round(sum(int(row.mastery_percent or 0) for row in skill_rows) / max(len(skill_ids), 1)) if skill_ids else 0
                    next_lesson = Lesson.query.filter_by(learning_trajectory_id=course_row.course_id).filter(Lesson.status.in_(['planned', 'scheduled', 'draft'])).order_by(Lesson.lesson_date.asc().nullslast()).first()
                    course_cards.append({'course': course_row, 'mastery': mastery, 'next_lesson': next_lesson})
                course_mastery = course_cards[0]['mastery']
                course_next_lesson = course_cards[0]['next_lesson']
        ...
        children.append({
            ...
            , 'course_next_lesson': course_next_lesson
